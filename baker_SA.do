...
qui: areg y i.year dd7 - dd23 dd25-dd48 if cohort!=0, a(id) robust
	...
	matrix B2wfeNt = (J(1,6,0)\Ball2wfeNt[(`T'+1)..(`T'+`DDneg'),1]\0\Ball2wfeNt[(`T'+`DDneg' + 1)..(rowsof(Ball2wfeNt)-1),1])		// Stack matrices, with a 0 for the effect in rel year -1
...
program define SAest, eclass
	...
	*** Step 1: Get the cohort-relative year specific effects
	quietly areg y i.year int_dd*, a(newid) robust 
...
set seed 1	
xtset, clear
bootstrap _b, reps(5) nowarn cluster(id) idcluster(newid) seed(1): SAest
